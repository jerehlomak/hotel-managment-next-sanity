...
import { User } from "@/app/models/booking";
...
const UserDetails = (props: { params: { id: string } }) => {
  ...
  const reviewSubmitHandler = async () => {
    ...
    if (!ratingText.trim().length || !ratingValue) {
        return toast.error('Please provide a rating and rating text')
    }
  ...
  // error handler
  if (error || userErrorData) throw new Error("Cannot fetch data");
  if (typeof userBooking === "undefined" && !isLoading)
    throw new Error("Cannot fetch data");
  if (typeof userData === "undefined" && !userLoadingData)
    throw new Error("Cannot fetch data");

  if (userLoadingData) return <LoadingSpinner />;
  ...
  return (
    <div className="container mx-auto px-2 md:px-4 py-10">
      <div className="grid md:grid-cols-12 gap-10">
        <div className="hidden md:block md:col-span-4 lg:col-span-3 shadow-lg h-fit sticky top-10 bg-[#eff0f2] text-black rounded-md px-6 py-4">
          <div className="md:w-[143px] w-38 h-38 md:h-[143px] mx-auto rounded-full overflow-hidden">
            <Image
              src={userData.image}
              alt={userData.name}
              width={143}
        ...
        <div className="md:col-span-8 lg:col-span-9">
            ...
            <ol
              className={`${
                currentNav === "bookings" ? "text-blue-600" : "text-gray-700"
                        ...
                        inline-flex mr-2 md:mr-5 items-center space-x-1 md:space-x-3
                    `}
            >
              <li
          ...
          {currentNav === "amount" ? (
            userBooking && <Chart userBookings={userBooking} />
          ) : (
            <></>
          )}
        </div>
